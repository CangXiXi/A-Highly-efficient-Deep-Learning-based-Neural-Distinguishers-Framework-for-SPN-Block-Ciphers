...
from tensorflow.keras.layers import Dense, Conv1D,Conv2D, Input, Reshape, Permute, Add, Flatten, BatchNormalization, Activation
...
def train_distinguisher(
        ...
        n_filters=32, reg_param=10 ** -5, lr_high=0.002, lr_low=0.0001, case=0
):
    n_rounds = cipher.get_n_rounds()
    cipher_name = type(cipher).__name__
    result_base_name = f'{wdir}_{cipher_name}_best_r_{n_rounds}'
    net = make_resnet(
        depth=depth, d1=n_neurons, d2=n_neurons, ks=kernel_size, num_filters=n_filters, reg_param=reg_param,
         word_size=cipher.get_word_size(), num_blocks=cipher.get_n_words()
    )
